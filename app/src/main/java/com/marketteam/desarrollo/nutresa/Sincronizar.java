...
public class Sincronizar extends AppCompatActivity {
    ...
    TextView pb1,ssin,cont,ssinr,contr ,fssin,fcont,tvt1,tvt2,tvt3,tvt4,tvt5,tvt6,tvt7,tvt8,tvt9,tvt10,tvt11,tvttf,tvttr,tvttv;
    static Sincronizar activityA;
    String ntact,ntsig;
    Context context;
    String encuestaAct,conteosin,conteosT,conteosinR,conteosTR,conteosFsin,conteosFT,codproy;
    StorageReference mStorageRef;
    Uri file;
    ProgressBar pbfotos;
    FirebaseFirestore db;
    ImageView iv;
    Button sincd,sincf,sincbr;
    ImageButton botonr;
    int ct1,ct2,ct3,ct4,ct5,ct6,ct7,ct8,ct9,ct10,ct11,cttf,cfsin,cttr,cttv,crss,ctx;
    ...
    protected void onStart() {
        ...
        sincbr = (Button) findViewById(R.id.buttonSincronizarR);
        botonr = (ImageButton) findViewById(R.id.ibCerrarSesion);
        String QueryConteosEnc = "select count(encuesta) as CE from t1 where estado=1 and sincronizada=0 or sincronizada is null";
        ...
        String QueryConteosEncR = "select count(encuesta) as CE from tr where sincronizada=0 or sincronizada is null";
        ArrayList<String>[] ConteoEncR = conGen.queryObjeto2val(getBaseContext(), QueryConteosEncR, null);
        String QueryConteosEncTR = "select count(encuesta) as CE from tr";
        ArrayList<String>[] ConteoEncTR = conGen.queryObjeto2val(getBaseContext(), QueryConteosEncTR, null);
        ...
        codproy = fg.getQ1("SELECT va FROM PA where pa='cproy'");
        conteosin = ConteoEnc[0].get(0);
        conteosT = ConteoEncT[0].get(0);
        conteosinR = ConteoEncR[0].get(0);
        conteosTR = ConteoEncTR[0].get(0);
        ...
        ssinr = (TextView) findViewById(R.id.tVTotalSSN2);
        contr = (TextView) findViewById(R.id.tVTotalFN2);
        ...
        ssinr.setText(conteosinR);
        contr.setText(conteosTR);
    ...
    public void irselcuest(View v) {
        ...
        Intent lista = new Intent(this, SeleccionCuestionario.class);
        startActivity(lista);
    }
    ...
    public void sincronizar(View vista) {
        ...
        sincbr.setClickable(false);
        Toast.makeText(getBaseContext(),"Sincronizando los datos, por favor espere", Toast.LENGTH_LONG).show();
        sincronizarTablasVF();
    }

    public void sincronizarR(View vista) {
        //SincronizaVF s = new SincronizaVF(getBaseContext());
        sincf.setClickable(false);
        sincd.setClickable(false);
        botonr.setClickable(false);
        sincbr.setClickable(false);
        Toast.makeText(getBaseContext(),"Sincronizando los rechazos, por favor espere", Toast.LENGTH_LONG).show();
        sincronizarRechazVF();
    }
            ...
            for (int r = 0; r < registros.length; r++) {
                ...
                if (net == true) {
                    ...
                    if (f.exists() && f.length() > 0 && f != null) {
                        ...
                        if (file != null) {
                            final StorageReference picRef = mStorageRef.child(codproy + "/img/" + nombreFoto);
    ...
    public void sincronizarT1() {
        ...
        if (registros != null) {
            ...
            for (i = 0; i < registros.length; i++) {
                ...
                try {
                    final int finalI = i;
                    db.collection("t1_" + codproy).document(registros[i].get(0)).set(regis).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                    ...
                    db.collection("t1_" + codproy).document(s).set(regis).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT2(String enct2) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t2_" + codproy).document(Encuesta).set(tabla2).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct2++;
                ...
                db.collection("t2_" + codproy).document(s).set(tabla2).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarTablasG(final String ntabla, String enctx) {
        ArrayList<String>[] campos = conGen.queryObjeto(context, "pragma table_info(" + ntabla + ")", null);
        ArrayList<String>[] registros = conGen.queryObjeto(context, "SELECT * FROM " + ntabla + " where encuesta='" + enctx + "'", null);
        Map<String, Object> tablag = new HashMap<>();
        Map<String, Object> regis;
        String Encuesta = enctx;
        if (registros != null) {
            for (int i = 0; i < registros.length; i++) {
                regis = new HashMap<>();
                for (int x = 0; x < campos.length; x++) {
                    regis.put(campos[x].get(1), registros[i].get(x));
                }
                tablag.put(enctx + "_" + i, regis);
            }
            try {
                db.collection(ntabla + "_" + codproy).document(Encuesta).set(tablag).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ctx++;
                        TextView tv = retornartext(2);
                ...
                final String s = Encuesta;
                db.collection(ntabla + "_" + codproy).document(s).set(tablag).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(context, "La sincronización ha fallado en " + ntabla, Toast.LENGTH_SHORT).show();
                    }
                });
            } finally {
                tablag.clear();
                sincronizarTablasG(ntsig,encuestaAct);
            }
        } else {
            sincronizarTablasG(ntsig,encuestaAct);
        }
    ...
    public void sincronizarT3(String enct3) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t3_" + codproy).document(Encuesta).set(tabla3).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct3++;
                ...
                final String s = Encuesta;
                db.collection("t3_" + codproy).document(s).set(tabla3).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT4(String enct4) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t4_" + codproy).document(Encuesta).set(tabla4).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct4++;
                ...
                final String s = Encuesta;
                db.collection("t4_" + codproy).document(s).set(tabla4).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT5(String enct5) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t5_" + codproy).document(Encuesta).set(tabla5).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct5++;
                ...
                final String s = Encuesta;
                db.collection("t5_" + codproy).document(s).set(tabla5).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT6(String enct6) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t6_" + codproy).document(Encuesta).set(tabla6).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct6++;
                ...
                final String s = Encuesta;
                db.collection("t6_" + codproy).document(s).set(tabla6).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT7(String enct7) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t7_" + codproy).document(Encuesta).set(tabla7).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct7++;
                ...
                final String s = Encuesta;
                db.collection("t7_" + codproy).document(s).set(tabla7).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT8(String enct8) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t8_" + codproy).document(Encuesta).set(tabla8).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct8++;
                ...
                final String s = Encuesta;
                db.collection("t8_" + codproy).document(s).set(tabla8).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT9(String enct9) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t9_" + codproy).document(Encuesta).set(tabla9).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct9++;
                ...
                final String s = Encuesta;
                db.collection("t9_" + codproy).document(s).set(tabla9).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT10(String enct10) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t10_" + codproy).document(Encuesta).set(tabla10).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct10++;
                ...
                final String s = Encuesta;
                db.collection("t10_" + codproy).document(s).set(tabla10).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarT11(String enct11) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("t11_" + codproy).document(Encuesta).set(tabla11).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        ct11++;
                ...
                final String s = Encuesta;
                db.collection("t11_" + codproy).document(s).set(tabla11).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
...
//              String nombre = registros[i].get(1);
            ...
            try {
                db.collection("tf_" + codproy).document(Encuesta).set(tablaf).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        cttf++;
                ...
                final String s = Encuesta;
                db.collection("tf_" + codproy).document(s).set(tablaf).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
    ...
    public void sincronizarTFV2(String enctf) {
        boolean fotosubida = false;
        ArrayList<String>[] registros = conGen.queryObjeto(context, "SELECT * FROM tf where encuesta='" + encuestaAct + "' order by ID2", null);
        Map<String, Object> tablaf = new HashMap<>();
        final String Encuesta = enctf;
...
//              String nombre = registros[i].get(1);
            ...
            try {
                db.collection("tf_" + codproy).document(Encuesta).set(tablaf).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(@NonNull Void T) {
                        ...
                        tv.setText("" + cttf);
                        String encu = Encuesta;
                        String cesR = "" + (Integer.parseInt(conteosinR) - cttr);
                        ssinr.setText(cesR);
                        if (cttf==crss) {
                            sincf.setClickable(true);
                            sincd.setClickable(true);
                            botonr.setClickable(true);
                            sincbr.setClickable(true);
                            Toast.makeText(context, "Sincronizacion de Rechazos Finalizada", Toast.LENGTH_LONG).show();
                        }
                        operaciones.queryNoData("UPDATE tr SET sincronizada='1' WHERE encuesta='" + encu + "'");
                    }
                });
            } catch (Exception e) {
                final String s = Encuesta;
    ...
    public void sincronizarTR() {
        ArrayList<String>[] registros = conGen.queryObjeto(context, "SELECT * FROM tr where sincronizada='0' or sincronizada is null", null);
        crss = Integer.parseInt(fg.getQ1("Select count(encuesta) from tr where  sincronizada='0' or sincronizada is null"));
        if (registros != null) {
            for (int i = 0; i < registros.length; i++) {
                encuestaAct = registros[i].get(0);
                Map<String, Object> regis = new HashMap<>();
                ...
                try {
                    db.collection("tr_" + codproy).document(registros[i].get(0)).set(regis).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(@NonNull Void T) {
                            cttr++;
                    ...
                    final String s = registros[i].get(0);
                    db.collection("tr_" + codproy).document(s).set(regis).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
            ...
            botonr.setClickable(true);
            sincbr.setClickable(true);
            Toast.makeText(context, "No hay Rechazos por sincronizar", Toast.LENGTH_SHORT).show();
            Intent intX = new Intent(context, Sincronizar.class);
            intX.addFlags(FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intX);
        }
    ...
    public void sincronizarTV(String enctf) {
        ...
        if (registros != null) {
            ...
            try {
                db.collection("tv_" + codproy).document(Encuesta).set(tablaf).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(@NonNull Void T) {
                        cttv++;
                ...
                final String s = Encuesta;
                db.collection("tv_" + codproy).document(s).set(tablaf).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        //db.collection("tf").document(s).set(cv2);
